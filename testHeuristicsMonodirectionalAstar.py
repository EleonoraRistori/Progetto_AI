...
def test():
    ...
    plt.bar(X_axis - 0.3, mean_wrong, 0.3, label='A* algorithm using number of wrong tiles heuristic')
    plt.bar(X_axis, mean_man, 0.3, label='A* algorithm using Manhattan Distance')
    plt.bar(X_axis + 0.3, mean_LC, 0.3, label='A* algorithm using Manhattan Distance + Linear Conflicts')
